...
namespace Container {
namespace Iterator {
template <class _ValueType, class _PointerType = _ValueType*,
...
struct Iterator;
template <class T>
requires std::contiguous_iterator<T> struct Iterator<T>
    : public ContiguousIteratorBase<typename T::value_type, typename T::pointer,
                                    typename T::reference,
                                    typename T::difference_type> {
 private:
  using Base =
      ContiguousIteratorBase<typename T::value_type, typename T::pointer,
                             typename T::reference,
                             typename T::difference_type>;
 ...
 public:
  ...
  typename Base::reference operator*() const override { return *_iterator; }
  typename Base::pointer operator->() override { return &(*_iterator); }
...
struct GenericContiguousIterator {
 ...
 public:
  GenericContiguousIterator(const GenericContiguousIterator& other)
      : GenericContiguousIterator(new IterType(other._iterator)) {}
  ...
  GenericContiguousIterator& operator=(
      GenericContiguousIterator other) noexcept {
    std::swap(_iterator, other._iterator);
    return *this;
  }
  template <class T>
  ...
  GenericContiguousIterator(const T& iterator)
      : GenericContiguousIterator{static_cast<IterType*>(new Iterator<T>(iterator))} {}
  ~GenericContiguousIterator() { delete _iterator; }
  ...
  GenericContiguousIterator operator++(int) {
    return GenericContiguousIterator((*_iterator)++);
  }
  GenericContiguousIterator& operator--() {
    --(*_iterator);
    return *this;
  ...
  friend GenericContiguousIterator operator+(
      const GenericContiguousIterator& it, const int& ind) {
  ...
  reference operator[](const int& ind) { return *this += ind; }
