...
class Main : public wxFrame {
  ...
  std::vector<std::unique_ptr<StrategyBase>> CreateAllStrategyCombinations() {
    ...
    std::vector<std::unique_ptr<StrategyBase>> t_strats = CreateAllStrategyCombinations<T, V, Ts...>();
    std::vector<std::unique_ptr<StrategyBase>> u_strats = CreateAllStrategyCombinations<U, V, Ts...>();
    for (auto& ptr : t_strats) strats.push_back(std::move(ptr));
    for (auto& ptr : u_strats) strats.push_back(std::move(ptr));
    return strats;
  }
